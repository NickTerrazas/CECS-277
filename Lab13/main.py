...
def main_menu():
    print("-Tasklist-")
    print(f"Tasks to complete: {len(tasks)}") #FIX IMPORT ONCE TASKLIST IS COMPLETE
    ...
    choice = check_input.get_int("Enter choice: ", 1, 6)
    return choice
...
"""
def get_date():
    month = check_input.get_int("Enter month: ", 1, 12)
    day = check_input.get_int("Enter day: ", 1, 31)
    year = check_input.get_int("Enter year: ", 2000, 2100)
...
def main():
    while True:
        choice = main_menu()
        if choice == 1:
            #TODO waiting for tasklist to be complete to implement
        elif choice == 2:
            #TODO waiting for tasklist to be complete to implement
        elif choice == 3:
            #TODO waiting for tasklist to be complete to implement
